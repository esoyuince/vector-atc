...
const trafficRow=f=>[f.id,f.phase,...f.positionNm,f.altitudeFt,f.speedKt,f.headingDeg,f.verticalRateFpm,f.waitSeconds,f.command?[f.command.route,f.command.altitudeFt,f.command.speedKt,f.command.verticalRateFpm]:null,f.procedure.nextLeg?.fix||null,f.pilot?.unable||[]];
const compactRunway=r=>({id:r.id,active:r.active,elevation:r.elevation,lengthM:r.lengthM,headingTrue:r.headingTrue,reserved:r.reserved||null,reservations:r.reservations||[],occupants:r.occupants||[],completed:r.completed||0});
...
export function batchPlans(plan,size=24){
 ...
 while(start<plan.flights.length){let take=Math.min(size,plan.flights.length-start),batch;
  ...
  if(!take)throw new Error('TypeSafe request cannot fit the configured envelope.');batches.push(batch);start+=take;
 }
 return batches;
}
export function buildRequest(plan,model){
 ...
 for(const f of plan.flights){
  ...
  questions[f.id+'_speed']={type:'choice',instructions:common+'Choose target IAS in knots. Obey next-leg speed/max, sequence traffic and use 140-160 kt on final.',criteria:SPEED_CHOICES};
  questions[f.id+'_rate']={type:'choice',instructions:common+'Choose vertical-rate magnitude in ft/min; sign follows target altitude. Respect the next restriction, SID gradient and approach path; never descend into terrain.',criteria:RATE_CHOICES};
 }
 ...
 return {model,state:{...plan.state,policy:'Sole ATC; commands apply simultaneously. Pilot obeys physics and published procedure constraints; pilot.unable reports conflicts. No local traffic-avoidance controller. Prioritize predicted conflicts, terrain and occupied runways; resolve conflict pairs asymmetrically. Arrivals follow assigned STAR then matching ILS, using holds for sequencing and MISSED for go-around. Departures prefer requested SID but may use another offered SID for that runway. Altitudes are MSL; touchdown target is runway elevation, not zero. Route sequencing is autopilot; altitude/speed/rate are clearance targets. Ground aircraft move only with runway clearance. Keep route, numeric and runway choices mutually compatible; these questions are independent and share only this state. Balance queues. Emergency requests contain only affected aircraft; all other clearances remain in force.'},questions};
}
