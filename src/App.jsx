...
export default function App(){
 ...
 return <div className="app-shell autonomous">
  ...
  {state.trafficScope==='airborne-handoff-v1'&&<p className="replay-note">{language==='en'?'Airborne control':'Hava kontrolü'}: {state.flights.filter(f=>['arrival','approach','departure','takeoff'].includes(f.phase)).length} / {state.flights.length} · {language==='en'?'Scheduled handoffs':'Planlı katılım'}: {state.phases.pending||0}</p>}<ProcedureInfo state={state} flight={flight} language={language}/><main><Radar isReplay={state.isReplay} language={language} flights={state.flights} selected={selected} onSelect={setSelected} alerts={state.alerts} running={state.running&&!error&&!stale} runways={state.runways} showGround={showGround}/><DecisionPanel language={language} flight={flight} state={state} now={now}/></main>
