...
for(const [name,expected] of Object.entries(provenance.files))if(createHash('sha256').update(fs.readFileSync(path.join(root,name))).digest('hex')!==expected)throw Error('Source changed since last build: '+name);
