...
import fs from 'node:fs';import {createAirborneSimulation} from '../src/simulation.mjs';import {runtimeVersions,initialStateFingerprint,provenance,parseStudyManifest,STUDY_START_POLICY} from '../server/study-run.mjs';import {AIRBORNE_SCOPE} from '../src/traffic-lifecycle.mjs';
...
const m={schemaVersion:1,status:'frozen-local',executionStartPolicy:STUDY_START_POLICY,runId,scope:AIRBORNE_SCOPE,seed,sourceFingerprint:provenance.sourceFingerprint,versions:runtimeVersions(),requestedModel:'jev-1.13.0',initialStateSha256:await initialStateFingerprint(sim),stopping:{targetSimulatedSeconds:Number(simRaw),maxWallSeconds:Number(wallRaw),maxTotalInputTokens:Number(tokensRaw),stopForFavorableResults:false},preregistered:false,independentRuleReview:false,reviewPacketSha256:null,ruleReviewSha256:null};
