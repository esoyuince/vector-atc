...
export function sourceInventory(root) {
 ...
 const hashes=Object.fromEntries([...new Set(files)].sort().map(file=>[file,hashBytes(fs.readFileSync(path.join(root,file)))]));
 return {schemaVersion:1,sourceFingerprint:hashBytes(JSON.stringify(hashes)),files:hashes};
}
