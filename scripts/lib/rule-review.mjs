...
export function validateRuleReview(review){
 const errors=[];if(review?.schemaVersion!==1)errors.push('schemaVersion');if(review?.dataset!=='LTFM-SOUTH-v1')errors.push('dataset');if(review?.evaluationProtocol!==spec.protocolId)errors.push('evaluationProtocol');
 if(review?.status!=='approved')errors.push('status must be approved');const r=review?.reviewer??{};for(const key of ['name','role','reviewedAt'])if(typeof r[key]!=='string'||!r[key].trim())errors.push('reviewer.'+key);
